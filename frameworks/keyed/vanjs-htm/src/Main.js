...
const { html } = vanHTM({ htm, van })

let { add, state, derive } = van
...
let Row = ({_id, _label, _deleted}) => () => _deleted.val ? null : html`
  <tr id=${_id}>
    <td class="col-md-1">${_id}</td>
    <td class="col-md-4">
      <a onclick=${() => selected.val = _id}>${_label}</a>
    </td>
    <td class="col-md-1">
